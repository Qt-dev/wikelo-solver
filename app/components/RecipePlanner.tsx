...
function rewardTimingLabel(output: RecipeDto["outputs"][number]) {
  if (output.grantTiming === "mission_start") return output.kind === "blueprint" ? "Blueprint granted when mission starts" : "Granted when mission starts";
  if (output.grantTiming === "mission_completion") return output.kind === "blueprint" ? "Blueprint granted on completion" : output.quantity === 1 ? "Item awarded on completion" : "Items awarded on completion";
  return output.kind === "blueprint" ? "Blueprint reward" : "Mission reward";
}
      ...
      <section className={`planner-workspace ${recipeFocused ? "focused" : ""}`} aria-busy={loading}>
        ...
        <section className="detail-panel" id="recipe-details" aria-live="polite">
          {selectedRecipe && selectedSummary ? <>
            ...
            <header className="detail-title"><div><p className="eyebrow">Current recipe</p><h2>{selectedRecipe.name}</h2>{selectedRecipe.notForRelease && <p className="data-flag warning" aria-label="Not for release: present in game data but flagged as unavailable.">Not for release</p>}</div><div className="detail-actions"><div className="detail-progress"><span>{selectedSummary.readiness}% owned</span><div className="progress-track" aria-hidden="true"><span style={{ width: `${selectedSummary.readiness}%` }} /></div></div><div className="todo-add-control"><input aria-label="Number of completions to add" type="number" min="1" max="99" value={todoQuantity} onChange={(event) => setTodoQuantity(Math.max(1, Math.min(99, Number(event.target.value))))} /><button type="button" disabled={pendingTodos.has(selectedRecipe.id)} onClick={() => void updateTodo(selectedRecipe.id, (todos[selectedRecipe.id] ?? 0) + todoQuantity)}>Add to to-do</button></div></div></header>
            <section className="recipe-rewards" aria-labelledby="recipe-rewards-title"><div className="reward-heading"><div><p className="eyebrow">Granted by this mission</p><h3 id="recipe-rewards-title">Mission rewards</h3></div><span>{selectedRecipe.outputs.length} reward{selectedRecipe.outputs.length === 1 ? "" : "s"}</span></div><div className="reward-list">{selectedRecipe.outputs.map((output, index) => <article className={output.kind === "blueprint" && output.grantTiming === "mission_start" ? "mission-start-reward" : output.kind === "blueprint" ? "blueprint-reward" : ""} key={`${output.itemId ?? output.name}-${index}`}><span className="reward-quantity">{output.quantity}×</span><div className="reward-copy">{output.grantTiming === "mission_start" && <span className="reward-badge">Granted at mission start</span>}<strong>{output.name}</strong><small>{rewardTimingLabel(output)}</small>{output.externalUrl && <a href={output.externalUrl} target="_blank" rel="noreferrer">View blueprint on SCMDB <span aria-hidden="true">↗</span></a>}</div></article>)}</div></section>
            <div className="recipe-overview"><div><span>Reputation needed</span><strong>{selectedRecipe.reputationNeeded}</strong>{selectedRecipe.reputationNeededLabel && <p>{selectedRecipe.reputationNeededLabel}</p>}</div><div><span>Granted</span><strong>+{selectedRecipe.reputationGranted}</strong></div><div className="overview-cost"><span>Still to source</span><strong>{formatAuec(selectedSummary.valueAuec)} <small>aUEC</small></strong><p>{selectedSummary.complete ? "All needed units are priced." : `${selectedSummary.missingItemIds.length} needed item price${selectedSummary.missingItemIds.length === 1 ? " is" : "s are"} missing.`}</p></div></div>
