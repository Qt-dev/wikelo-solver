...
Assert-Equal 2 @($collectorNormalized.recipes).Count 'TheCollector contract discovery failed.'
$inlineRecipe = @($collectorNormalized.recipes | Where-Object gameRecipeId -EQ 'wikelo-inline')[0]
$fixedRecipe = @($collectorNormalized.recipes | Where-Object gameRecipeId -EQ 'wikelo-fixed')[0]
Assert-Equal 'wikelo-inline' $inlineRecipe.gameRecipeId 'TheCollector contract ID mismatch.'
Assert-Equal 'Ore' $inlineRecipe.components[0].name 'DataForge reference name resolution failed.'
Assert-Equal 'Reward Box' $inlineRecipe.output.name 'Reward reference name resolution failed.'
Assert-Equal 4 @($inlineRecipe.outputs).Count 'All reward items should be collected.'
Assert-Equal 'Reward Token' $inlineRecipe.outputs[1].name 'Second reward name resolution failed.'
Assert-Equal 20 $inlineRecipe.outputs[1].quantity 'Second reward quantity mismatch.'
Assert-Equal 'Cds Superheavy Helmet 01 Blueprint' $inlineRecipe.outputs[2].name 'Blueprint filename fallback failed.'
Assert-Equal 'blueprint' $inlineRecipe.outputs[2].kind 'Blueprint reward kind mismatch.'
Assert-Equal 'mission_start' $inlineRecipe.outputs[2].grantTiming 'Blueprint grant timing mismatch.'
Assert-Equal 'https://scmdb.net/?page=fab&fab=BP_CRAFT_Cds_Superheavy_Helmet_01' $inlineRecipe.outputs[2].externalUrl 'SCMDB blueprint URL mismatch.'
Assert-Equal 'Cds Superheavy Helmet 01' $inlineRecipe.outputs[3].name 'Crafted entity filename fallback failed.'
Assert-Equal 'Ore' $fixedRecipe.components[0].name 'Fixed template reference name resolution failed.'
Assert-Equal 7 $fixedRecipe.components[0].quantity 'Fixed template requirement quantity mismatch.'
